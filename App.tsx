...
const LikesGrid: React.FC<{ profiles: GamerProfile[]; onProfileClick: (p: GamerProfile) => void }> = ({ profiles, onProfileClick }) => (
    <div className="p-4 pt-24 pb-10 grid grid-cols-2 gap-4 overflow-y-auto h-full max-w-5xl mx-auto">
        {profiles.length === 0 ? (
...
// 2. Side Navigation (Discord Style)
const SideNav: React.FC<{ activeView: ActiveView; onNavigate: (v: ActiveView) => void }> = ({ activeView, onNavigate }) => {
    const items = [
        { id: 'swipe', icon: 'fa-gamepad', label: 'התאמות', color: 'text-green-400', bg: 'group-hover:bg-green-500' },
        { id: 'search', icon: 'fa-compass', label: 'חיפוש', color: 'text-green-400', bg: 'group-hover:bg-green-500' },
        { id: 'likes-you', icon: 'fa-heart', label: 'לייקים', color: 'text-red-400', bg: 'group-hover:bg-red-500' },
        { id: 'chat', icon: 'fa-comment', label: 'צ׳אט', color: 'text-blue-400', bg: 'group-hover:bg-blue-500' },
    ];
    
    return (
        <div className="fixed top-0 right-0 bottom-0 w-[72px] bg-[#1E1F22] z-[60] flex flex-col items-center py-3 gap-2 overflow-y-auto no-scrollbar shadow-xl">
             {/* Home / Profile Icon */}
             <div className="relative group w-12 h-12 flex items-center justify-center mb-2">
                 <div className={`absolute right-[-4px] w-1 bg-white rounded-l-lg transition-all duration-300 ${activeView === 'profile' ? 'h-8 opacity-100' : 'h-2 opacity-0 group-hover:opacity-50'}`}></div>
                 <button
                    onClick={() => onNavigate('profile')}
                    className={`w-12 h-12 rounded-[24px] overflow-hidden transition-all duration-300 group-hover:rounded-[16px] ${activeView === 'profile' ? 'rounded-[16px] ring-2 ring-dogame-primary' : ''} bg-dogame-surface hover:bg-dogame-primary`}
                >
             ...
             {items.map(item => {
                 ...
                 return (
                    <div key={item.id} className="relative group w-12 h-12 flex items-center justify-center">
                        {/* Pill Indicator */}
                        <div className={`absolute right-[-4px] w-1 bg-white rounded-l-lg transition-all duration-200 ${isActive ? 'h-8 opacity-100' : 'h-2 opacity-0 group-hover:h-5 group-hover:opacity-50'}`}></div>
                        
                        <button
                            onClick={() => onNavigate(item.id as ActiveView)}
                            className={`w-12 h-12 flex items-center justify-center transition-all duration-300 
                                ${isActive ? 'rounded-[16px] bg-dogame-primary text-white' : 'rounded-[24px] bg-dogame-surface text-gray-400 hover:rounded-[16px] hover:text-white ' + item.bg}`}
                        >
                            <i className={`fa-solid ${item.icon} text-xl transition-colors`}></i>
                        </button>
                    </div>
                 )
             })}
...
const SwipeView: React.FC<{ onLike: (p: GamerProfile) => void; userProfile: GamerProfile; gameFilter: string | null; onClearFilter: () => void }> = ({ onLike, userProfile, gameFilter, onClearFilter }) => {
    ...
    return (
        <div className="h-full w-full relative flex flex-col items-center justify-center pb-10 pt-16 px-4 max-w-xl mx-auto">
            ...
            {/* Action Buttons */}
            <div className="absolute bottom-4 w-full max-w-xs flex justify-center gap-8 px-4">
                <button 
...
const SearchView: React.FC<{ onSelect: (g: string) => void }> = ({ onSelect }) => {
    ...
    return (
        <div className="pt-20 px-4 h-full overflow-y-auto pb-10 max-w-5xl mx-auto">
...
const SubscriptionsView: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-pop backdrop-blur-sm" onClick={onClose}>
...
const App: React.FC = () => {
    ...
    return (
        <div className="h-screen w-full relative bg-dogame-bg text-dogame-text font-sans overflow-hidden">
            {/* Right Sidebar Navigation */}
            <SideNav activeView={activeView} onNavigate={handleNavigate} />

            {/* Main Content Area - Adjusted to not be covered by sidebar */}
            <div className="h-full w-full mr-[72px] flex flex-col transition-all duration-300">
                <Header 
                    viewTitle={activeView} 
                    onOpenSubscriptions={() => setIsSubModalOpen(true)}
                    onOpenProfile={() => { setViewingProfile(null); setActiveView('profile'); }}
                    onOpenChat={() => setActiveView('chat')}
                    showBackButton={activeView === 'profile' || (activeView === 'chat' && window.innerWidth < 768)}
