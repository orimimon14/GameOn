    ...
    return (
        <div className="h-full w-full overflow-y-auto relative pb-32">
            ...
            <div className="px-4 max-w-2xl mx-auto relative z-10 pt-16">
                ...
                <div className="flex flex-col items-center text-center mb-10">
                    <div className="relative w-40 h-40 mb-6 group">
                        ...
                        <div className="absolute inset-0 p-1.5 rounded-full overflow-hidden">
                            ...
                            <div className="relative z-10 w-full h-full rounded-full border-[6px] border-background overflow-hidden shadow-2xl bg-surface">
                                <img src={editedProfile.image} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                            </div>
                        </div>
                        
                        {isOwnProfile && (
                            <button onClick={() => { setImageTarget('image'); setIsImageModalOpen(true); }} className="absolute bottom-2 right-2 z-20 w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center border-4 border-background hover:scale-110 transition-transform shadow-glow">
                    ...
                    <div className="w-full">
                        {isEditing ? (
                            <input value={editedProfile.name} onChange={e => setEditedProfile({...editedProfile, name: e.target.value})} className="bg-white/5 border-b-2 border-primary/50 text-4xl font-black text-white text-center focus:outline-none focus:border-primary w-full py-2 mb-2 italic uppercase" />
                        ) : (
                        ...
                        {isEditing ? (
                            <div className="flex justify-center items-center gap-2 mt-1">
                                <span className="text-text-muted text-lg font-bold">גיל</span>
                                <input 
                                    type="number" 
                                    value={editedProfile.age} 
                                    onChange={e => setEditedProfile({...editedProfile, age: parseInt(e.target.value) || 0})} 
                                    className="bg-white/5 border-b border-white/20 text-lg text-white text-center focus:outline-none focus:border-primary w-16" 
                                />
                            </div>
                        ) : (
                            <p className="text-primary text-xl font-bold tracking-widest">{editedProfile.age} גיימר</p>
                        )}
                    </div>
                </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-xl rounded-[24px] p-6 border border-white/10 mb-8 shadow-xl flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <div className="text-right">
                                ...
                                <p className="text-text-muted text-sm">{ownedBackgrounds.length} פריטי סטייל</p>
                            </div>
                            <button 
                        ...
                        {profile.bannerImage && (
                            <div className="flex items-center justify-between">
                                <div className="text-right">
                                    ...
                                    <p className="text-text-muted text-sm">הצג את הרקע הזה בכל האתר</p>
                                </div>
                                <button 
                ...
                {/* Main Card */}
                <div className="bg-surface/40 backdrop-blur-2xl rounded-[32px] p-8 shadow-2xl border border-white/10 mb-8">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-black text-white italic uppercase">ביוגרפיה</h3>
                        {isOwnProfile && !isEditing && (
                             <button onClick={() => setIsEditing(true)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all shadow-sm">
                    ...
                    {isEditing ? (
                        <textarea value={editedProfile.bio} onChange={e => setEditedProfile({...editedProfile, bio: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-2xl p-4 text-white focus:outline-none focus:border-primary min-h-[150px]" />
                    ) : (
                    ...
                    {isOwnProfile && isEditing && (
                        <div className="grid grid-cols-2 gap-4 mt-8">
                            <button onClick={handleSave} className="py-4 bg-primary text-white rounded-2xl font-black italic uppercase shadow-glow hover:scale-[1.02] transition-transform">שמור שינויים</button>
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                     <div className="bg-white/5 backdrop-blur-lg p-6 rounded-[24px] border border-white/10 text-center shadow-lg">
                         <span className="text-text-muted text-xs font-black uppercase tracking-widest block mb-2">רמת מיומנות</span>
                         <span className="text-2xl font-black text-white italic uppercase">{profile.skillLevel}</span>
                     </div>
                     <div className="bg-white/5 backdrop-blur-lg p-6 rounded-[24px] border border-white/10 text-center shadow-lg">
                         <span className="text-text-muted text-xs font-black uppercase tracking-widest block mb-2">ספריית משחקים</span>
                ...
                <div className="mb-12">
                    ...
                    <div className="space-y-4">
                        {editedProfile.games.map((game, idx) => (
                            <div key={idx} className="bg-surface/30 backdrop-blur-xl p-6 rounded-[24px] border border-white/10 flex items-start gap-5 hover:bg-white/5 transition-all text-right shadow-xl group">
                                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary/20 to-premium/20 flex items-center justify-center shrink-0 border border-white/10 group-hover:rotate-6 transition-transform">
                                    <i className={`fa-solid ${game.icon} text-primary text-2xl`}></i>
                                </div>
                ...
                {isOwnProfile && !isEditing && (
                    <div className="flex flex-col gap-4 mb-12">
                        <button 
                            onClick={() => setIsCollectionModalOpen(true)}
                            className="w-full py-4 bg-gradient-to-r from-primary to-premium text-white font-black italic uppercase rounded-[20px] shadow-glow hover:scale-[1.01] transition-all flex items-center justify-center gap-3"
                        >
                            <i className="fa-solid fa-palette"></i>
                            <span>שנה סטייל מהאוסף</span>
                        </button>
                        <button className="w-full py-4 text-danger font-black italic uppercase hover:bg-danger/10 rounded-[20px] transition-colors border border-danger/10">
                            התנתק מהמערכת
                        </button>
                    </div>
                )}
