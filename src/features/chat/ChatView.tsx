...
export const ChatView: React.FC<ChatViewProps> = ({ matches }) => {
    ...
    return (
        <div className="flex h-full w-full bg-transparent relative z-10">
            ...
            <div className={`flex-1 flex flex-col h-full dark:bg-background/40 bg-white/40 backdrop-blur-md border-l dark:border-white/5 border-gray-200 transition-all ${!selectedMatchId ? 'hidden md:flex' : 'flex'}`}>
                {activeMatch ? (
                    <>
                        {/* Header */}
                        <div className="h-20 flex items-center px-6 border-b dark:border-white/5 border-gray-200 shrink-0">
                            <button onClick={() => setSelectedMatchId(null)} className="md:hidden ml-4 dark:text-white text-text-inverse">
                                <i className="fa-solid fa-arrow-right text-xl"></i>
                            </button>
                            <div className="flex items-center gap-4 text-right flex-1">
                                <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-primary shadow-glow">
                                    <img src={activeMatch.image} className="w-full h-full object-cover" alt={activeMatch.name} />
                                </div>
                                <div>
                                    <h3 className="font-bold dark:text-white text-text-inverse text-lg">{activeMatch.name}</h3>
                            ...
                            <form onSubmit={e => {e.preventDefault(); handleSend()}} className="flex gap-3">
                                ...
                                <button 
                                    ...
                                    className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${
                                        newMessage.trim() ? 'bg-primary text-white shadow-glow' : 'bg-gray-400/20 text-gray-400'
                                    }`}
                                >
            ...
            <div className={`w-full md:w-96 flex flex-col h-full dark:bg-surface/20 bg-white/20 backdrop-blur-md ${selectedMatchId ? 'hidden md:flex' : 'flex'}`}>
                <div className="p-6 h-20 border-b dark:border-white/5 border-gray-200 flex items-center justify-between">
                    <h2 className="text-2xl font-black italic uppercase dark:text-white text-text-inverse">הודעות</h2>
                    <div className="flex gap-2">
                        <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center">
                            <i className="fa-solid fa-filter text-xs text-primary"></i>
                        </div>
                    </div>
                </div>
                        ...
                        matches.map(m => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`w-full p-4 rounded-2xl flex items-center gap-4 transition-all ${selectedMatchId === m.id ? 'bg-primary text-white shadow-lg' : 'hover:bg-white/10 dark:text-white text-text-inverse'}`}
                                >
                                    ...
                                    <div className="flex-1 text-right min-w-0">
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            <span className={`text-[10px] ${selectedMatchId === m.id ? 'text-white/70' : 'text-text-muted'}`}>{lastMsg?.timestamp || 'חדש'}</span>
                                        </div>
                                        <p className={`text-sm truncate ${selectedMatchId === m.id ? 'text-white/80' : 'text-text-muted'}`}>
