...
עדכון יישום (2026-07-16): בהתאם ל-ADR-037, שכבת ה-webhook מומשה מול RevenueCat
(`paymentWebhook` מאמת `Authorization` header, ממפה `app_user_id` → uid וכותב
`subscriptions/{uid}`; `onSubscriptionUpdated` גוזר isPro). ספק הסליקה שמאחורי
RevenueCat (web) נותר פתוח תחת ADR-017; בחנויות — StoreKit/Play Billing דרך
RevenueCat. הפעלה בפועל דורשת רק: חשבון RevenueCat, סוד `PAYMENT_WEBHOOK_SECRET`,
ו-`system/config.billing = { enabled, webPurchaseUrl }`.
...
### ADR-044: Pricing & Monetization Structure (research-backed)

**Status:** Accepted (2026-07-19) — על בסיס ניתוח מודל עסקי + מחקר שוק מאומת (16.7.2026).

**עוגני מחיר (מנויים):**

| מסלול | מחיר | הערות |
|---|---|---|
| Pro שבועי | ₪14.90 | לקהל צעיר ללא מחויבות חודשית; 82% ממנויי אפליקציות גיימינג נמכרים שבועי |
| Pro חודשי (עוגן) | ₪29.90 | ללא שינוי — ניתוח רגישות: ההכנסה שטוחה בטווח ₪20–35; מתחת ל-Tinder Plus (₪39.90) ול-Discord Nitro (≈₪37) |
| Pro שנתי | ₪119.90 | ≈₪10/חודש (-67%), לפי נורמת הקטגוריה (שנתי ≈ פי 3 מחודשי); טווח בדיקה עתידי עד ₪199.90 |
| Elite | ₪49.90/חודש | שלב 3 בלבד (10K+ MAU): עדיפות בתור, בוסט חודשי כלול, קוסמטיקה בלעדית |

**ניסיון חינם:** 14 יום למסלול חודשי/שנתי (נתוני RevenueCat: ניסיונות ארוכים ממירים ~42.5% מול 25.5% בקצרים; מנויי-ניסיון משתמרים טוב יותר). A/B עתידי: 30 יום.

**כלכלת מטבעות (שלב 2 — דורש ביטול ההסתייגות ב-ADR-018/034 לגבי מכירת מטבעות):**

| חבילה | מחיר | מטבעות | אג'/מטבע |
|---|---|---|---|
| טעימה | ₪4.90 | 100 | 4.90 |
| קטנה | ₪10.90 | 250 | 4.36 |
| פופולרית | ₪19.90 | 550 | 3.62 |
| ענקית | ₪34.90 | 1,200 | 2.91 (-41%) |

מחירי הפריטים הקיימים בקטלוג נשארים (מכוילים נכון מול שווי המטבע). המטבע נשאר קוסמטי בלבד — לקח Reddit (סגרה את מערכת המטבעות שלה ב-2023 בגלל מורכבות ומטבע שקנה דברים לא-מוחשיים); אין pay-to-win.

**רכישות חד-פעמיות (שלב 2):** בוסט (שעה בראש הערימה) ₪9.90 / 3 ב-₪24.90 / 200 מטבעות; "הזמנה בוערת" ₪6.90 / 5 ב-₪27.90 / 140 מטבעות. ~30% מתחת למחירי Tinder בישראל (בוסט ₪13.90–21.90, סופר-לייק ~₪12).

**פרסומות:** rewarded בלבד ("צפה וקבל מטבעות"), שלב 3, רק אם צריך; interstitials אסורים. הסרת פרסומות תהיה הטבת Pro אם/כאשר יופעלו.

**סדר הפעלה:** שלב 1 (השקה) מנויים בלבד → שלב 2 (~10K MAU) מטבעות + רכישות חד-פעמיות → שלב 3 Elite + rewarded ads.
...
| ADR-044 | תמחור: Pro ₪14.90/שבוע · ₪29.90/חודש · ₪119.90/שנה + ניסיון 14 יום; מטבעות ₪4.90–34.90 (5→2.9 אג'); בוסט ₪9.90; הזמנה בוערת ₪6.90; הפעלה בשלבים | Product |
